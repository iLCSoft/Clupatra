...
inline bool inRange(int i) {
  return ((unsigned int)(i - Min) <= (unsigned int)(Max - Min));
}

/** fast check if integer is not in a given range [Min,Max] */
template <int Min, int Max>
inline bool notInRange(int i) {
  return ((unsigned int)(i - Min) > (unsigned int)(Max - Min));
}
...
public:
  ...
  ~PtrVector() {
    if (_isOwner)
      for (typename vec::iterator i = vec::begin(), end = vec::end(); i != end; delete *i++)
        ; //++i ) delete *i ;
  }
...
public:
  ...
  ~PtrList() {
    if (_isOwner)
      for (typename base::iterator i = base::begin(), end = base::end(); i != end; delete *i++)
        ; //++i ) delete *i ;
  }
  void setOwner(bool val = true) { _isOwner = val; }
