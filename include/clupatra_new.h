...
struct ClupaHit {
  ...
  ClupaHit() : layer(-1), zIndex(-1), phiIndex(-1), lcioHit(0), pos(0., 0., 0.) {}
  int layer;
  int zIndex;
  int phiIndex;
...
struct ZSort {
  inline bool operator()(const Hit* l, const Hit* r) { return (l->first->pos.z() < r->first->pos.z()); }
};
...
public:
  ...
  inline bool operator()(Hit* h0, Hit* h1) {
    ...
    if (std::abs(h0->Index0 - h1->Index0) > 1)
      return false;
...
struct LCIOTrackConverter {
  ...
  LCIOTrackConverter()
      : UsePropagate(false), CaloFaceBarrelID(lcio::ILDDetID::ECAL), CaloFaceEndcapID(lcio::ILDDetID::ECAL_ENDCAP) {}
...
struct IMarlinTrkFitter {
  ...
  IMarlinTrkFitter(MarlinTrk::IMarlinTrkSystem* ts, double maxChi2Increment = DBL_MAX)
      : _ts(ts), _maxChi2Increment(maxChi2Increment) {}
...
/** Try to add hits from hLV (hit lists per layer) to the cluster. The cluster needs to have a fitted KalTrack
 * associated to it. Hits are added if the resulting delta Chi2 is less than dChiMax - a maxStep is the maximum number
 * of steps (layers) w/o successfully merging a hit.
 */
int addHitsAndFilter(CluTrack* clu, HitListVector& hLV, double dChiMax, double chi2Cut, unsigned maxStep,
                     ZIndex& zIndex, bool backward = false, MarlinTrk::IMarlinTrkSystem* trkSys = 0);
//------------------------------------------------------------------------------------------
...
struct TrackInfoStruct {
  TrackInfoStruct()
      : zMin(0.), zAvg(0.), zMax(0.), startsInner(false), isCentral(false), isForward(false), isCurler(false) {}
  float zMin;
  float zAvg;
  float zMax;
  bool startsInner;
...
public:
  ...
  TrackSegmentMerger(float chi2Max, MarlinTrk::IMarlinTrkSystem* trksystem, float b)
      : _chi2Max(chi2Max), _trksystem(trksystem), _b(b) {}
  ...
  inline bool operator()(nnclu::Element<lcio::Track>* h0, nnclu::Element<lcio::Track>* h1) {
    ...
    if (lthf0 + overlapRows <= lthl1 && lthf1 + overlapRows <= lthl0)
      return false;
    ...
    //      const lcio::TrackState* ts = ( outward ? trk->getTrackState( lcio::TrackState::AtLastHit  ) :
    //      trk->getTrackState( lcio::TrackState::AtFirstHit  ) ) ;
    const lcio::TrackState* ts = trk->getTrackState(lcio::TrackState::AtFirstHit);

    streamlog_out(DEBUG3) << " *******  TrackSegmentMerger : will extrapolate track "
                          << (outward ? " outwards\t" : " inwards\t") << lcio::lcshort(trk) << "     vs:  [" << std::hex
                          << oth->id() << std::dec << "]" << std::endl;
...
public:
  ...
  inline bool operator()(nnclu::Element<lcio::Track>* h0, nnclu::Element<lcio::Track>* h1) {
    ...
    streamlog_out(DEBUG2) << "TrackCircleDistance::operator() : " << trk0->id() << " <-> " << trk1->id()
                          << "  (  ti0->zAvg > ti1->zAvg ) = " << (ti0->zAvg > ti1->zAvg) << std::endl;
    ...
    streamlog_out(DEBUG2) << "TrackCircleDistance::operator() : " << trk0->id() << " <-> " << trk1->id()
                          ...
                          << " (  dtl > 2.  * _dCut  &&  std::abs( tl0 + tl1 ) > 1.e-2  ) = "
                          << (dtl > 2. * _dCut && std::abs(tl0 + tl1) > 1.e-2) << std::endl;
    ...
    // streamlog_out( DEBUG2 ) << "TrackCircleDistance::operator() : " <<  trk0->id() << " <-> "  << trk1->id()
    //  			      << " (  std::abs( d0 ) < rIP &&  std::abs( z0 ) < rIP  && std::abs( d1 ) < rIP &&
    //  std::abs( z1 ) < rIP     ) "
    //  			      << (  std::abs( d0 ) < rIP &&  std::abs( z0 ) < rIP  && std::abs( d1 ) < rIP &&
    //  std::abs( z1 ) < rIP     )
    //  			      << std::endl ;
    ...
    streamlog_out(DEBUG2) << "TrackCircleDistance::operator() : " << trk0->id() << " <-> " << trk1->id()
                          ...
                          << (std::abs(z0) < rIP && std::abs(z1) < rIP) << std::endl;
    ...
    streamlog_out(DEBUG2) << "TrackCircleDistance:: operator() : " << trk0->id() << " <-> " << trk1->id()
                          ...
                          << (dr < _dCut * std::abs(r0) && distMS < _dCut * std::abs(r0)) << " dr : " << dr
                          << " _dCut * std::abs( r0 ) " << _dCut * std::abs(r0) << " distMS :" << distMS << std::endl;
...
public:
  ...
  void time(unsigned index) { _clocks[index] = clock(); }
  void start() { time(0); }
