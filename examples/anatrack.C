...
plot6hists(TFile* fi, const char* dir, const std::string& fileName, char* title, char* h1, char* h2, char* h3, char* h4,
           char* h5, char* h6 = 0) {
  ...
  c1->cd(pad++);
  hName = path + std::string(h1);
  hist = (TH1D*)fi->Get(hName.c_str());
  if (hist)
    hist->Draw();
  c1->cd(pad++);
  hName = path + std::string(h2);
  hist = (TH1D*)fi->Get(hName.c_str());
  if (hist)
    hist->Draw();
  c1->cd(pad++);
  hName = path + std::string(h3);
  hist = (TH1D*)fi->Get(hName.c_str());
  if (hist)
    hist->Draw();
  c1->cd(pad++);
  hName = path + std::string(h4);
  hist = (TH1D*)fi->Get(hName.c_str());
  if (hist)
    hist->Draw();
  c1->cd(pad++);
  hName = path + std::string(h5);
  hist = (TH1D*)fi->Get(hName.c_str());
  if (hist)
    hist->Draw();
