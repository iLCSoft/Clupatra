...
bool sort_CDot(const CDot& c0, const CDot& c1) { return c0.Dot > c1.Dot; }

//-------------------------------------------------------------------------------

int addHitsAndFilter(CluTrack* clu, HitListVector& hLV, double dChi2Max, double chi2Cut, unsigned maxStep,
                     ZIndex& zIndex, bool backward, MarlinTrk::IMarlinTrkSystem* trkSys) {
  ...
  streamlog_out(DEBUG3) << " ======================  addHitsAndFilter():  - layer " << layer
  ...
  if (trk == 0) {
    streamlog_out(DEBUG3) << "  addHitsAndFilter called with null pointer to MarlinTrk  - won't do anything "
                          << std::endl;
    return nHitsAdded;
  }
  ...
  if (streamlog_level(DEBUG3)) {
    ...
    for (auto p : hits)
      streamlog_out(DEBUG3) << "\n"
                            ...
                            << p.first->getPosition()[0] << ", " << p.first->getPosition()[1] << ", "
  ...
  if (trkSys &&
      backward) { //==================== only active if called with _trkSystem pointer ============================
    ...
    streamlog_out(DEBUG2) << " ---- addHitsAndFilter : will smooth back to " << i << "th  hit - size of clu "
                          << clu->size() << std::endl;
    ...
    streamlog_out(DEBUG3) << "  -- addHitsAndFilter(): smoothed track segment : " << MarlinTrk::errorCode(smoothed)
                          << " using track state : " << ts << " ---- chi2: " << chi2 << "  ndf " << ndf << std::endl;
  ...
  while (step < maxStep + 1) {
    ...
    streamlog_out(DEBUG3) << "  -- addHitsAndFilter(): looked for intersection - "
                          << "  Step : " << step << "  at layer: " << layer
                          << "   intersects: " << MarlinTrk::errorCode(intersects) << "  next xing point : " << xv;
    ...
    if (intersects == IMarlinTrack::success) { // found a crossing point
      ...
      if (bestHit != 0) {
        ...
        streamlog_out(DEBUG3) << " ************ bestHit " << bestHit
                              ...
                              << " chi2: " << ch2Min << " chi2Cut: " << chi2Cut << std::endl;
        ...
        if (ch2Min < chi2Cut) {
          ...
          streamlog_out(DEBUG3) << " *****       assigning left over hit : "
                                << errorCode(addHit) //<< hPos << " <-> " << xv
                                << " dist: " << (hPos - xv).r() << " chi2: " << ch2Min << "  hit errors :  rphi="
                                << sqrt(bestHit->first->lcioHit->getCovMatrix()[0] +
...
bool addHitAndFilter(int detectorID, int layer, CluTrack* clu, HitListVector& hLV, double dChi2Max, double chi2Cut) {
  ...
  streamlog_out(DEBUG2) << "  ============ addHitAndFilter(): looked for intersection - "
                        << "  detector : " << detectorID << "  at layer: " << layer
                        << "  intersects: " << MarlinTrk::errorCode(intersects) << "  next xing point : " << xv;
  ...
  if (intersects == IMarlinTrack::success) { // found a crossing point
    ...
    if (bestHit != 0) {
      ...
      if (ch2Min < chi2Cut) {
        ...
        streamlog_out(DEBUG2) << " *****       assigning left over hit : "
                              << errorCode(addHit) //<< hPos << " <-> " << xv
                              << " dist: " << (hPos - xv).r() << " chi2: " << ch2Min << "  hit errors :  rphi="
                              << sqrt(bestHit->first->lcioHit->getCovMatrix()[0] +
                                      bestHit->first->lcioHit->getCovMatrix()[2])
                              << "  z= " << sqrt(bestHit->first->lcioHit->getCovMatrix()[5]) << std::endl;
...
void create_n_clusters(Clusterer::cluster_type& hV, Clusterer::cluster_list& cluVec, unsigned n) {
  ...
  for (int l = tpcNRow - 1; l >= 0; --l) {
    ...
    for (std::list<CDot>::iterator it = cDot.begin(); it != cDot.end(); ++it) {
      ...
      streamlog_out(DEBUG2) << " I : " << it->I << " J : " << it->J << " d : " << it->Dot << std::endl;
      unsigned i = it->I;
      unsigned j = it->J;
      ...
      if (cluIdx.find(i) == cluIdx.end() && hitIdx.find(j) == hitIdx.end()) {
        ...
        streamlog_out(DEBUG2) << " **** adding to cluster : " << it->I << " hit  : " << it->J << " d : " << it->Dot
                              << std::endl;
      }
    }
  }

  streamlog_out(DEBUG) << " create_three_clusters  --- clu[0] " << clu[0]->size() << " clu[1] " << clu[1]->size()
                       << " clu[2] " << clu[2]->size() << std::endl;
...
void create_three_clusters(Clusterer::cluster_type& hV, Clusterer::cluster_list& cluVec) {
  ...
  for (int l = tpcNRow - 1; l >= 0; --l) {
    ...
    streamlog_out(DEBUG) << " create_three_clusters  ---  layer " << l << "  h0 : " << p[0] << "  h1 : " << p[1]
                         << "  h2 : " << p[2];
    //			       << std::endl ;
    ...
    for (std::list<CDot>::iterator it = cDot.begin(); it != cDot.end(); ++it) {
      ...
      streamlog_out(DEBUG) << " I : " << it->I << " J : " << it->J << " d : " << it->Dot << std::endl;
      ...
      if (cluIdx.find(i) == cluIdx.end() && hitIdx.find(j) == hitIdx.end()) {
        ...
        streamlog_out(DEBUG) << " **** adding to cluster : " << it->I << " hit  : " << it->J << " d : " << it->Dot
                             << std::endl;
      }
    }
...
void create_two_clusters(Clusterer::cluster_type& clu, Clusterer::cluster_list& cluVec) {
  ...
  for (int l = tpcNRow - 1; l >= 0; --l) {
    ...
    streamlog_out(DEBUG) << " create_two_clusters  ---  layer " << l << "  h0 : " << p0 << "  h1 : " << p1;
    //			       << std::endl ;
  ...
  streamlog_out(DEBUG1) << " create_two_clusters  --- clu0 " << clu0->size() << " clu1 " << clu1->size() << std::endl;
...
start:
  ...
  if (clu->size() < 3) {
    ...
    streamlog_out(ERROR) << " IMarlinTrkFitter::operator() : cannot fit cluster track with less than 3 hits ! "
                         << std::endl;
  ...
  if (code != MarlinTrk::IMarlinTrack::success) {
    ...
    streamlog_out(ERROR) << "  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> IMarlinTrkFitter :  problem fitting track "
                         << " error code : " << MarlinTrk::errorCode(code) << std::endl;
  }
  ...
  if (isFirstFit && (1. * hitsInFit.size()) / (1. * nHit) < 0.2) { // fixme: parameter
    ...
    streamlog_out(DEBUG4) << "  >>>>>>  IMarlinTrkFitter :  small number of hits used in fit " << hitsInFit.size()
                          << "/" << nHit << " = " << (1. * hitsInFit.size()) / (1. * nHit)
                          << " refit with larger max chi2 increment:  " << maxChi2 << std::endl;
    delete trk;
...
#define use_fit_at_last_hit 0
...
#else // get the track state at the last hit by propagating from the last(first) constrained fit position (a la
      // MarlinTrkUtils)
      ...
      if (code != MarlinTrk::IMarlinTrack::success) {
        ...
        streamlog_out(DEBUG6) << "  >>>>>>>>>>> LCIOTrackConverter :  could not get TrackState at last Hit !!?? "
                              << std::endl;
      }
      ...
      code = mtrk->propagateToLayer(layerID, last_constrained_hit, *tsCA, chi2, ndf, sensorID,
      ...
      if (code == MarlinTrk::IMarlinTrack::no_intersection) {
        ...
        encoder[lcio::LCTrackerCellID::side()] =
            (lHit->getPosition()[2] > 0. ? lcio::ILDDetID::fwd : lcio::ILDDetID::bwd);
      ...
      if (code != MarlinTrk::IMarlinTrack::success) {
        ...
        streamlog_out(DEBUG6) << "  >>>>>>>>>>> LCIOTrackConverter :  could not get TrackState at calo face !!?? "
                              << std::endl;
      }
      ...
      if (code != MarlinTrk::IMarlinTrack::success) {
        ...
        streamlog_out(DEBUG6) << "  >>>>>>>>>>> LCIOTrackConverter :  could not extrapolate TrackState to IP !!?? "
                              << std::endl;
      }
      ...
      streamlog_out(WARNING) << "  >>>>>>>>>>> LCIOTrackConverter::operator()  -  hitsInFitEmpty ! - nHits " << nHit
                             << std::endl;
    }
