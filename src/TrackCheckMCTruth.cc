...
TrackCheckMCTruth::TrackCheckMCTruth() : Processor("TrackCheckMCTruth"), _nRun(0), _nEvt(0) {
  ...
  registerInputCollections(LCIO::SIMTRACKERHIT, "SimTrackerHitCollection",
                           "Name of the input collections with SimTrackerHits", _sthColNames, sthVec);

  registerInputCollection(LCIO::LCRELATION, "TrackMCPRelation",
                          "Name of the input collections with LCRelation Tracks to MCParticles", _relTrkMCPColName,
                          std::string("TrackRelation"));

  registerInputCollection(LCIO::LCRELATION, "MCPTrackRelation",
                          "Name of the input collections with LCRelation MCParticles to tracks", _relMCPTrkColName,
  ...
  registerOutputCollection(LCIO::TRACK, "SplitTrackCollection", "Name of the output collections with split Tracks",
                           _splitColName, std::string("ClupatraSplitTracks"));

  registerOutputCollection(LCIO::TRACK, "MergedTrackCollection", "Name of the output collections with merged Tracks",
                           _mergedColName, std::string("ClupatraMergedTracks"));

  registerProcessorParameter("HitFractionSplit",
                             "maximal fractions of hits that are allowed to be missing for a split track",
                             _hitFractionSplit, float(0.05));

  registerProcessorParameter("HitFractionMerged",
                             "maximal fractions of hits that are allowed to be missing for a merged track",
                             _hitFractionMerged, float(0.05));
...
void TrackCheckMCTruth::init() {
  ...
  if (_ptRange.size() != 2) {
    ...
    throw Exception(
        "TrackCheckMCTruth::init()  processor parameter 'ptRange' needs to have two values (min and max pt in GeV)");
  }
...
void TrackCheckMCTruth::processRunHeader(LCRunHeader*) { _nRun++; }
...
void TrackCheckMCTruth::processEvent(LCEvent* evt) {
  ...
  streamlog_out(DEBUG4) << " ===== found " << mcpTrksFound->size() << " tracks of " << mcpTracks->size() << std::endl;
