...
In find_smallest(In first, In last, Pred p, double& d) {
...
void printAndSaveTrack(const EVENT::LCObject* o) {
  ...
  if (trk && DebugTracks::col) {
    ...
    streamlog_out(MESSAGE) << " =========== added copy of track to debug collection with current size: "
                           << DebugTracks::col->getNumberOfElements() << " ==========" << std::endl;
  }
}
...
void printTrackerHit(const EVENT::LCObject* o) {
  ...
  if (hit == 0) {
    ...
    streamlog_out(MESSAGE) << " --- TrackerHit: " << *hit << "\n --- delta Chi2 = " << hit->ext<DChi2>()
                           << "\n --- cov. matrix = " << hit->getCovMatrix()[0] << ", " << hit->getCovMatrix()[2]
                           << ", " << hit->getCovMatrix()[5] << "  " << std::endl;
  }
}
//----------------------------------------------------------------
void printSimTrackerHit(const EVENT::LCObject* o) {
  ...
  if (hit == 0) {
    ...
    streamlog_out(ERROR) << " printSimTrackerHit : dynamic_cast<SimTrackerHit*> failed for LCObject : " << o
                         << std::endl;
    return;
    ...
    streamlog_out(MESSAGE) << " --- SimTrackerHit: " << *hit << "\n"
                           << " MCParticle: " << lcshort(hit->getMCParticle()) << std::endl;
  }
}
//----------------------------------------------------------------
...
ClupatraProcessor::ClupatraProcessor() : Processor("ClupatraProcessor"), _trksystem(0), _tpc(0) {

  // modify processor description
  _description = "ClupatraProcessor : nearest neighbour clustering seeded pattern recognition";

  registerInputCollection(LCIO::TRACKERHIT, "TPCHitCollection", "Name of the tpc hit input collections", _colName,
                          "AllTPCTrackerHits");

  registerOutputCollection(LCIO::TRACK, "OutputCollection", "Name of the output collection with final TPC tracks",
                           _outColName, std::string("ClupatraTracks"));

  registerOutputCollection(LCIO::TRACK, "SegmentCollectionName",
                           "Name of the output collection that has the individual track segments", _segmentsOutColName,
                           std::string("ClupatraTrackSegments"));

  registerProcessorParameter("DistanceCut", "Cut for distance between hits in mm for the seed finding", _distCut,
                             (float)40.0);

  registerProcessorParameter("CosAlphaCut",
                             "Cut for max.angle between hits in consecutive layers for seed finding - NB value should "
                             "be smaller than 1 - default is 0.9999999 !!!",
                             _cosAlphaCut, (float)0.9999999);

  registerProcessorParameter(
      "NLoopForSeeding",
      "number of seed finding loops - every loop increases the distance cut by DistanceCut/NLoopForSeeding", _nLoop,
      (int)4);

  registerProcessorParameter("MinimumClusterSize", "minimum number of hits per cluster", _minCluSize, (int)6);

  registerProcessorParameter("DuplicatePadRowFraction", "allowed fraction of hits in same pad row per track",
                             _duplicatePadRowFraction, (float)0.1);
  ...
  registerProcessorParameter("Chi2Cut", "the maximum chi2-distance for which a hit is considered for merging ",
                             _chi2Cut, (float)100.);
  ...
  registerProcessorParameter("PadRowRange", "number of pad rows used in initial seed clustering", _padRowRange,
                             (int)12);

  registerProcessorParameter(
      "NumberOfZBins", "number of bins in z over total length of TPC - hits from different z bins are nver merged",
      _nZBins, (int)150);

  registerProcessorParameter(
      "MinLayerFractionWithMultiplicity",
      "minimum fraction of layers that have a given multiplicity, when forcing a cluster into sub clusters",
      _minLayerFractionWithMultiplicity, (float)0.5);

  registerProcessorParameter(
      "MinLayerNumberWithMultiplicity",
      "minimum number of layers that have a given multiplicity, when forcing a cluster into sub clusters",
      _minLayerNumberWithMultiplicity, (int)3);

  registerProcessorParameter("TrackStartsInnerDist",
                             "maximum radial distance [mm] from inner field cage of first hit, such that the track is "
                             "considered to start at the beginning ",
                             _trackStartsInnerDist, (float)25.);

  registerProcessorParameter("TrackEndsOuterCentralDist",
                             "maximum radial distance [mm] from outer field cage of last hit, such that the track is "
                             "considered to end at the end ",
                             _trackEndsOuterCentralDist, (float)25.);

  registerProcessorParameter(
      "TrackEndsOuterForwardDist",
      "maximum distance in z [mm] from endplate of last hit, such that the track is considered to end at the end ",
      _trackEndsOuterForwardDist, (float)40.);

  registerProcessorParameter("TrackIsCurlerOmega",
                             "minimum curvature omega of a track segment for being considered a curler",
                             _trackIsCurlerOmega, (float)0.001);
  ...
  registerProcessorParameter("pickUpSiHits", "try to pick up hits from Si-trackers", _pickUpSiHits, bool(false));
  ...
  registerOptionalParameter("SITDetectorName", "name of the SIT-like inner Si-tracking barrel detector ",
                            _sitDetectorName, std::string("SIT"));

  registerOptionalParameter("VXDDetectorName", "name of the VXD-like inner Si-tracking barrel detector ",
                            _vxdDetectorName, std::string("VXD"));

  registerProcessorParameter(
      "CreateDebugCollections",
      "optionally create some debug collection with intermediate track segments and used and unused hits",
      _createDebugCollections, bool(false));

  registerProcessorParameter("TrackSystemName",
                             "Name of the track fitting system to be used ( DDKalTest, aidaTT, ... )", _trkSystemName,
                             std::string("DDKalTest"));

  registerProcessorParameter(
      "CaloFaceBarrelID",
      "system ID of the subdetector at the calorimeter face in the barrel - default: lcio::ILDDetID::ECAL=20 ",
      _caloFaceBarrelID, (int)20);

  registerProcessorParameter(
      "CaloFaceEndcapID",
      "system ID of the subdetector at the calorimeter face in the endcap - default: lcio::ILDDetID::ECAL=29 ",
      _caloFaceEndcapID, (int)29);
}
...
void ClupatraProcessor::processRunHeader(LCRunHeader*) { _nRun++; }
...
void ClupatraProcessor::processEvent(LCEvent* evt) {
  ...
  try {
    col = dynamic_cast<LCCollection*>(evt->getCollection(_colName));

  } catch (lcio::DataNotAvailableException& e) {

    streamlog_out(WARNING) << " input collection not in event : " << _colName << "   - nothing to do  !!! "
                           << std::endl;
  ...
  for (int i = 0; i < nHit; ++i) {
    ...
    if (fabs(th->getPosition()[2]) > driftLength)
      continue;
    ...
    streamlog_out(DEBUG) << "  ch->layer = idDec( th )[ LCTrackerCellID::layer() ] = " << ch->layer << " - CellID0 "
                         << th->getCellID0() << std::endl;
  ...
  streamlog_out(DEBUG2) << "  added  " << nncluHits.size() << "  tp hitsInLayer - > size " << hitsInLayer.size()
                        << std::endl;
  ...
  streamlog_out(DEBUG5)
      << "===============================================================================================\n"
      << "   first step of Clupatra algorithm: find seeds with NN-clustering  in " << _nLoop
      << " loops - max dist = " << _distCut << " \n"
      << "===============================================================================================\n";
  ...
  for (int nloop = 1; nloop <= _nLoop; ++nloop) {
    ...
    while (outerRow >= _minCluSize) { //_padRowRange * .5 ) {
      ...
      streamlog_out(DEBUG3) << "  -------- search seeds with distCut=" << nloop * dcut << " starting in row "
                            << outerRow << " with padrow range " << _padRowRange << " - found " << sclu.size()
                            << " seed clusters " << std::endl;

      for (Clusterer::cluster_list::iterator icv = sclu.begin(), end = sclu.end(); icv != end; ++icv) {

        int nHitsAdded = 0;

        //	streamlog_out( DEBUG4 ) <<  " call fitter for seed cluster with " << (*icv)->size() << " hits " <<
        //std::endl ;
  ...
  if (do_global_reclustering) {
    ...
    double rhoMaxInnerHits =
        (_tpc->rMinReadout + 0.67 * (_tpc->rMaxReadout - _tpc->rMinReadout)) / dd4hep::mm; // FIXME: make parameter

    streamlog_out(DEBUG5) << "  ===========================================================================\n"
                          << "      recluster in leftover hits - outside a clyinder of :  z =" << zMaxInnerHits
                          << " rho = " << rhoMaxInnerHits << "\n"
                          << "  ===========================================================================\n"
                          << std::endl;
    ...
    while (outerRow > 0) {
      ...
      for (int iRow = outerRow; iRow > minRow; --iRow) {
        ...
        streamlog_out(DEBUG) << "      hit candidates in row " << iRow << " : " << hitsInLayer[iRow].size()
                             << std::endl;
        ...
        for (HitList::iterator hlIt = hitsInLayer[iRow].begin(), end = hitsInLayer[iRow].end(); hlIt != end; ++hlIt) {
          streamlog_out(DEBUG) << "      hit candidate for reclustering " << (*hlIt)->first
                               << " ( std::abs( (*hlIt)->first->pos.z() ) > zMaxInnerHits  ||  "
                                  "(*hlIt)->first->pos.rho() >  rhoMaxInnerHits )  "
                               << (std::abs((*hlIt)->first->pos.z()) > zMaxInnerHits ||
                                   (*hlIt)->first->pos.rho() > rhoMaxInnerHits)
                               << std::endl;
      ...
      streamlog_out(DEBUG) << "   reclusterd in the range : " << outerRow << " - " << minRow << " found "
                           << loclu.size() << " clusters " << std::endl;
      ...
      for (Clusterer::cluster_list::iterator it = loclu.begin(), end = loclu.end(); it != end; ++it) {
        ...
        if (float(mult[5]) / mult[0] >= _minLayerFractionWithMultiplicity &&
            mult[5] > _minLayerNumberWithMultiplicity) {
        ...
        else if (float(mult[4]) / mult[0] >= _minLayerFractionWithMultiplicity &&
                 mult[4] > _minLayerNumberWithMultiplicity) {
        ...
        else if (float(mult[3]) / mult[0] >= _minLayerFractionWithMultiplicity &&
                 mult[3] > _minLayerNumberWithMultiplicity) {
        ...
        else if (float(mult[2]) / mult[0] >= _minLayerFractionWithMultiplicity &&
                 mult[2] > _minLayerNumberWithMultiplicity) {
        ...
        } else if (float(mult[1]) / mult[0] >= _minLayerFractionWithMultiplicity &&
                   mult[1] > _minLayerNumberWithMultiplicity) {
  ...
  //=======================================================================================================================
  //  try again to gobble up hits at the ends ....   - does not work right now, as there are no fits  for the
  //  clusters....
  //=======================================================================================================================

  // streamlog_out( DEBUG5 ) << " ===========     gobble up leftover hits at the ends for "  <<  cluList.size() << "
  // clusters " << std::endl ;
  ...
  if (merge_split_segments) {
    ...
    for (unsigned l = 0; l < 2; ++l) { // do this twice ....
      ...
      streamlog_out(DEBUG4) << " ===== merged track segments - # cluster: " << incSegCluVec.size() << " from "
                            << incSegVec.size() << " incomplete track segments "
                            << "  ============================== " << std::endl;
      ...
      for (TrackClusterer::cluster_vector::iterator it = incSegCluVec.begin(); it != incSegCluVec.end(); ++it) {
        ...
        for (TrackClusterer::cluster_type::iterator itC = incSegClu->begin(); itC != incSegClu->end(); ++itC) {
          ...
          for (lcio::TrackerHitVec::const_iterator it1 = trk->getTrackerHits().begin(),
                                                   END = trk->getTrackerHits().end();
               it1 != END; ++it1) {
        ...
        // icov[ 9] = 1e2 ;
        // icov[14] = 1e2 ;
        // int result = createFinalisedLCIOTrack( mTrk, hits, track, !MarlinTrk::IMarlinTrack::backward, icov, _bfield,
        // _dChi2Max ) ;
        // //int result = createFinalisedLCIOTrack( mTrk, hits, track, ! MarlinTrk::IMarlinTrack::backward, icov,
        // _bfield,  _dChi2Max ) ;
        // // ???
  ...
  if (merge_curler_segments) {
    ...
    streamlog_out(DEBUG4) << " ===== merged tracks - # cluster: " << curSegCluVec.size() << " from " << tsCol->size()
                          << " track segments " << "  ============================== " << std::endl;
    ...
    for (TrackClusterer::cluster_vector::iterator it = curSegCluVec.begin(); it != curSegCluVec.end(); ++it) {
      ...
      if (copyTrackSegments) {
        ...
        if (ts)
          trk->addTrackState(new TrackStateImpl(*ts));
        ...
        double RMin = (ts ? sqrt(ts->getReferencePoint()[0] * ts->getReferencePoint()[0] +
                                 ts->getReferencePoint()[1] * ts->getReferencePoint()[1])
                          : 0.0);
        trk->setRadiusOfInnermostHit(RMin);

        streamlog_out(DEBUG2) << "   create new merged track from bestTrack parameters - ptr to MarlinTrk : "
                              << trk->ext<MarTrk>()
                              << "   with subdetector hit numbers  : " << trk->subdetectorHitNumbers()[0] << " , "
                              << trk->subdetectorHitNumbers()[1] << std::endl;
    ...
    for (TrackClusterer::element_vector::iterator it = curSegVec.begin(); it != curSegVec.end(); ++it) {
      ...
      if ((*it)->second == 0) {
        ...
        if (copyTrackSegments) {
          ...
          streamlog_out(DEBUG2) << "   create new track from existing LCIO track  - ptr to MarlinTrk : "
                                << t->ext<MarTrk>() << std::endl;
  ...
  if (_createDebugCollections) {
    ...
    for (LCIterator<TrackImpl> it(outCol); TrackImpl* trk = it.next();) {
      ...
      bool isCentral =
          std::abs(lhPos.rho() - r_outer) < _trackEndsOuterCentralDist; // last hit close to outer field cage
  ...
  if (writeQualityTracks) {
    ...
    for (LCIterator<TrackImpl> it(outCol); TrackImpl* trk = it.next();) {
      ...
      streamlog_out(DEBUG4) << " gsl_cdf_chisq_Q( " << trk->getChi2() << ", " << (double)trk->getNdf()
                            << " ) = " << prob << " hitsInFit=" << hitsInFit << ", hitsInTrack =" << hitsInTrack
                            << " # TrackStates=" << nTrackStates << std::endl;
...
void ClupatraProcessor::pickUpSiTrackerHits(EVENT::LCCollection* trackCol, LCEvent* evt) {
  ...
  streamlog_out(DEBUG3) << " ************ pickUpSiTrackerHits() called - nTracks : " << trackCol->getNumberOfElements()
                        << std::endl;
  ...
  if (parameterSet("SITHitCollection")) {
    ...
    streamlog_out(DEBUG2) << " --  pickUpSiTrackerHits - read SIT hits from collection " << _sitColName
                          << "  with size = " << it.size() << "\n";
  ...
  for (std::map<int, std::list<TrackerHit*>>::iterator it = hLMap.begin(), End = hLMap.end(); it != End; ++it) {
    ...
    streamlog_out(DEBUG3) << "  *****  sensor: " << encoder.valueString() << " - nHits: " << it->second.size()
                          << std::endl;
  }
  ...
  try {
    ...
    streamlog_out(ERROR) << " ==== in pickUpSiTrackerHits() : could not get detector info (ZPlanarData) for SIT  "
                         << std::endl;
    return;
  }
  ...
  if (!tv) {
    streamlog_out(ERROR) << " *** pickUpSiTrackerHits() :  dynamic_cast<LCCollectionVec*>(trackCol)  failed !! "
                         << std::endl;
    return;
  }
  ...
  for (LCIterator<TrackImpl> it(trackCol); TrackImpl* trk = it.next();) {
    ...
    for (int lx = nLayers - 1; lx >= 0; --lx) {
      ...
      streamlog_out(DEBUG3) << " *******  pickUpSiTrackerHits - intersection with SIT/VXD layer " << layer
                            ...
                            << " sensorID: " << encoder.valueString() << std::endl;

      if (intersects == MarlinTrk::IMarlinTrack::success) {

        std::list<TrackerHit*>& hL = hLMap[sensorID];

        streamlog_out(DEBUG3) << "    **** found candidate hits : " << hL.size() << "         for point " << point
                              << std::endl;
        ...
        if (bestIt == hL.end() || min > maxDist) {
          ...
          streamlog_out(DEBUG3) << " ######### no close by hit found !! "
                                << " (bestIt == hL.end())" << (bestIt == hL.end()) << " (min  > maxDist)"
                                << (min > maxDist) << std::endl;
        ...
        streamlog_out(DEBUG3) << "    ****  best matching hit : " << dd4hep::rec::Vector3D((*bestIt)->getPosition())
                              << "         added : " << MarlinTrk::errorCode(addHit) << "   deltaChi2: " << deltaChi
                              << std::endl;
...
void ClupatraProcessor::check(LCEvent* evt) {
  ...
  for (LCIterator<Track> it(evt, _outColName); EVENT::Track* trk = it.next();) {
    ...
    //    streamlog_out( DEBUG2 ) <<  lcshort( trk ) <<  ", " << ts0 <<  ", " << ts1 <<  ", " << ts2 <<  ", " << ts3  <<
    //    std::endl ;
    streamlog_out(DEBUG3) << " -- " << ts0 << ", " << ts1 << ", " << ts2 << ", " << ts3 << std::endl;

    if (!ts0 || !ts1 || !ts2 || !ts3)

      streamlog_out(ERROR) << " clupatra track w/ missing track state : " << lcshort(trk) << "  ts0-ts3 : " << ts0
                           << ", " << ts1 << ", " << ts2 << ", " << ts3 << std::endl;
  }
...
void ClupatraProcessor::end() {
  ...
  streamlog_out(MESSAGE) << "ClupatraProcessor::end()  " << name() << " processed " << _nEvt << " events in " << _nRun
                         << " runs " << std::endl;
}
