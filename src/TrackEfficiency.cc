...
TrackEfficiency::TrackEfficiency() : Processor("TrackEfficiency"), _nRun(0), _nEvt(0) {

  // modify processor description
  _description = "TrackEfficiency: analysis plots for Tracks";

  registerInputCollection(LCIO::MCPARTICLE, "MCParticlesCollection", "Name of the input collections with MCParticles",
                          _mcpColName, std::string("MCParticlesSkimmed"));

  registerInputCollection(LCIO::SIMTRACKERHIT, "SimTrackerHitCollection",
                          "Name of the input collections with SimTrackerHits", _sthColName,
                          std::string("TPCCollection"));

  registerInputCollection(LCIO::LCRELATION, "TrackMCTruthRelation",
                          "Name of the input collections with LCRelation Tracks to MCParticles", _t2mColName,
                          std::string("TrackMCTruthRelation"));

  registerInputCollection(LCIO::LCRELATION, "MCTruthTrackRelation",
                          "Name of the input collections with LCRelation MCParticles to Tracks", _m2tColName,
                          std::string("MCTruthTrackRelation"));

  registerInputCollection(LCIO::TRACK, "TrackCollection", "Name of the input collections with  Tracks", _trkColName,
  ...
  registerProcessorParameter("PtRange", "Min and max value of pt range [GeV]", _ptRange, ptRange);
  ...
  registerProcessorParameter("CosThetaCut", "Maximum costheta of examined MCParticles sample", _cosTheta, double(0.99));

  registerProcessorParameter("PCut", "Minimum momentum of examined MCParticles sample (in GeV)", _pCut, double(1.0));
  ...
  registerProcessorParameter("RequiredPurity",
                             ...
                             _minPurity, double(0.75));

  registerProcessorParameter(
      "RequiredSiHits", "minimum required hits on Si detectors in order to consider the track found", _minHits, int(4));
}
...
void TrackEfficiency::init() {
  ...
  if (_ptRange.size() != 2) {
    ...
    throw Exception(
        "TrackEfficiency::init()  processor parameter 'ptRange' needs to have two values (min and max pt in GeV)");
  }
...
void TrackEfficiency::processRunHeader(LCRunHeader*) { _nRun++; }
...
void TrackEfficiency::processEvent(LCEvent* evt) {
  ...
  while (MCParticle* mcp = mcpIt.next()) {
    ...
    if (std::abs(mcp->getCharge()) < 0.5)
      cut = false; // silent cut
    if (cut)
  ...
  for (int ii = 0, nn = mcpTracks->getNumberOfElements(); ii < nn; ++ii) {
    ...
    if (trkV.size() > 0) {
      ...
      for (unsigned i = 0; i < wV.size(); ++i) {
        ...
        streamlog_out(DEBUG2) << "          nhit  : " << ((Track*)trkV[i])->getTrackerHits().size()
                              << " ---- weight : " << wV[i] << std::endl;
      ...
      //       APPLY_CUT( DEBUG4, cut,  std::abs( dom )  <  (5.*eom)  ) ;              // require 5 sigma on omega only
      //       for now APPLY_CUT( DEBUG4, cut,  std::abs( dtL  ) <  (3.*etL)  ) ;
      ...
      if (!cut) {
        ...
        streamlog_out(DEBUG3) << " pt: " << pt << " dpt: " << dpt << " ept : " << ept << " theta "
                              << 180. * atan(1. / tL) / M_PI << std::endl;
      }
  ...
  streamlog_out(DEBUG) << "---  histogramming time: " << double(end - start) / double(CLOCKS_PER_SEC) << std::endl;
}
...
void TrackEfficiency::end() {
  ...
  streamlog_out(MESSAGE) << " processed " << _nEvt << " events in " << _nRun << " runs " << std::endl;
}
