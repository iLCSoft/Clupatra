...
DebugDigiProcessor::DebugDigiProcessor() : Processor("DebugDigiProcessor"), _nRun(0), _nEvt(0) {
  ...
  registerProcessorParameter(
      "SubDetectorIDs", "The IDs of the subdetectors corresponding to the hit collections in TrackerHitCollectionNames",
      _subDetIDs, subDetIDs);

  StringVec hitColNames;
  hitColNames.push_back(std::string("TPCTrackerHits"));

  registerProcessorParameter("TrackerHitCollections",
                             "The tracker hit collection names - needs to run parallel to SubDetectorIDs", _hitColNames,
                             hitColNames);
}
...
void DebugDigiProcessor::init() {
  ...
  if (_subDetIDs.size() != _hitColNames.size()) {
    ...
    throw Exception("DebugDigiProcessor::init() - processor parameters SubDetectorIDs and TrackerHitCollections do not "
                    "have the same size !");
  }
...
void DebugDigiProcessor::processRunHeader(LCRunHeader*) { _nRun++; }
